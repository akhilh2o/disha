...
class QuizController extends Controller
{
    ...
    public function exam(Exam $exam)
    {
        ...
        if (!$exam->questions->count() > 0) {
            return back()->withErrors("Something went wrong! Please contact to Administrator.");
        }
    ...
    public function fillPDFFile($exam, $file, $outputFilePath)
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            $fpdi->Text(47, 253, auth()?->user()?->lastest_course?->issue_date ?? date('d-m-Y'));
            // insert image at position x,y,w,h (in mm)
            if (app()->environment() === 'local') {
                $fpdi->Image("https://cdn.vectorstock.com/i/preview-1x/82/99/no-image-available-like-missing-picture-vector-43938299.jpg", 165, 50, 23.3, 34);
            } else {
                if (auth()?->user()?->avatar) {
